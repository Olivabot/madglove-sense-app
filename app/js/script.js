document.querySelector('#connect').addEventListener('click', function () {
    ...
    .then(char => {
        ...
        statusElem.textContent = 'Connected! Waiting for data...';

        // Start notifications
        return characteristic.startNotifications().then(() => {
            characteristic.addEventListener('characteristicvaluechanged', event => {
                const rawData = new TextDecoder().decode(event.target.value);
                debugElem.textContent += `Raw Data: ${rawData}\n`;

                // Parse the received CSV-formatted data
                ...
                if (values.length >= 15) {
                    ...
                    // Display the human-readable data
                    dataElem.innerHTML = `
    ...
    .catch(error => {
        statusElem.textContent = 'Error occurred!';
